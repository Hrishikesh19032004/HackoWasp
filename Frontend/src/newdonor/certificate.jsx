...
export const MyPlugin = ({ organizationName }) => {
  const [donorName, setDonorName] = useState(""); // State to hold donor name
  ...
  const handleNamePrompt = () => {
    ...
    if (name) {
      setDonorName(name); // Update the donor name state with the input value
    } else {
  ...
  return (
    <div id="webcrumbs" ref={certificateRef}>
      ...
      {/* Download button */}
      <button
        onClick={handleNamePrompt} // Trigger name input prompt
        style={{
          marginTop: "16px",
      ...
      >
        Enter Donor Name & Download Certificate
      </button>
